...
import ServiceData from "../../data/service/ServiceMain15.json";
import { slugify } from '../../utils';


const AllData = ServiceData;

const ServiceProp14= ({colSize, serviceStyle, itemShow, marginTop}) => {
				...
				<div className={`${colSize} ${topMargin(index)}`} key={index} >
					<div className={`services-grid ${serviceStyle}`}>
						...
						<div className="content">
							...
							<p className='innovate-list-paraABC'>{data.description}</p>
						</div>
					</div>
			 	</div>
			))}
