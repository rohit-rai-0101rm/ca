...
import ServiceData from "../../data/service/ServiceMain7.json";
import { slugify } from '../../utils';


const AllData = ServiceData;

const ServicePropSeven= ({colSize, serviceStyle, itemShow, marginTop}) => {
				...
				<div className={`${colSize} ${topMargin(index)}`} key={index} >
					<div className={`services-grid ${serviceStyle}`}>
						...
						<div className="content">
							...
							<p className='innovate-list-para' to={process.env.PUBLIC_URL + `/service-details/${slugify(data.description)}`}>{data.description}</p>
						</div>
					</div>
			 	</div>
			))}
...
export default ServicePropSeven ;